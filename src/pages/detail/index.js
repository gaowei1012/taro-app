...
export default class Detail extends Component {
    ...
    // 页面刚加载时执行
    componentDidMount  = () => {
        // 当该页面首次加载，会从路由里面的参数传递过来 id 值
        this.setState({
            goodsId: this.$router.params.id, // 拿到不同商品传过来的id值
        })
    };

    async getGoodsInfo(goodsId) {
        const res = await detailApi.getProductInfo({id: goodsId})
        if (res.status == 'ok') {
            ...
            this.setState({
                ...
                console.log(`${res.data}, ${imgList}, ${res.data.specifications}`)
            })
        }
    };
    ...
    join = async () => {
        ...
        if (this.state.detail.mode_id == 3 && (this.state.detail.enabled != 1 || this.state.sale_stock == 0)) {
            Taro.showToast({
                title: '商品已售完',
                icon: 'none'
            });
            ...
            this.props.dispatch({
                ...
                payload: {
                    items: [...this.props.items, {
                        ...
                        name: detail.name,
                        product_id: detail.market_price,
    ...
    chooseSize = (e) => {
        const { has_stock, id, name } = e.currentTaget.dataset;
        // 只有当 has_stock == 1 时 才能选择
        if (has_stock == 1) {
            // 当前选择商品时，点击两次可以取消选择
            // 清空 重新添加
    ...
    goToPage = (e) => {
        ...
        if (Taro.getEnv() === Taro.ENV_TYPE.WEB) {
            Taro.navigateTo({
                url: e.currentTaget.dataset.url,
            })
        } else {
            Taro.switchTab({
                url: e.currentTaget.dataset.url,
            ...
            <View className="detail-page">
                ...
                <View className="container">
                    ...
                    <View className="goods-info">
                        <View className="chapter-head">
                            优质评价（ {detail.comments && detail.comments.total}
                            ）
                        </View>
                    ...
                    <View className="goods-info">
                        <View className="chapter-head">
                            服务说明 { /* <image className="icon"  alt="" src="../../images/icons/icon35.png" /> */}
                        </View>
                ...
                <View className="detail-bottom-btns">
                    ...
                    <View className="nav" data-url="/pages/cart/index" onClick={this.goToPage}>
                        ...
                        衣袋 {items.length > 0 && <View className="zan-badge__count">{items.length}</View>}
                    </View>
