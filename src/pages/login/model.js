...
export default {
    ...
    effects: {
        * login(_, { call, put, select }) {
            const { code, mobile, } = yield select(state => state.lgoin)
            ...
            if (res.status === 'ok') {
                ...
                setTimeout (() => {
                    Taro.navigateBack(); // 返回上一级
                }, 1000);
            }
        },
        * sendSms(_, { call, put, select }) {
            const { mobile } = yield select(state => state.lgoin);
            const res = yield call(login.sendSms, { mobile });
            if(res.status == 'ok') {
                yield put({ type: 'save', payload: { sending: 1, erroMessage: ''} });
            } else {
                yield put({ type: 'sava', payload: { sending: 2, erroMessage: res.error && res.error.message }});
            }
        },
